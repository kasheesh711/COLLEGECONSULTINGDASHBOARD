  ...
  return (
    <div className="space-y-4">
      {students.map((student) => {
        ...
        <article
          key={student.slug}
          className={clsx("rounded-[1.85rem] border p-5 shadow-[0_2px_4px_rgba(21,40,61,0.03),0_18px_36px_rgba(21,40,61,0.06)]", getQueueTone(student))}
        >
          <div className="grid gap-4 xl:grid-cols-[minmax(0,1.25fr)_minmax(320px,0.9fr)] xl:items-stretch">
            <div className="grid h-full gap-4 rounded-[1.75rem] bg-white/88 p-5 lg:grid-cols-[minmax(0,1fr)_220px]">
              <div className="min-w-0 space-y-4">
                ...
                <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-[var(--foreground-soft)]">
                  <span>{student.familyLabel}</span>
                  <span>{student.pathwayLabel}</span>
                  <span>{student.currentPhase}</span>
                  <span>{student.tier}</span>
                </div>
                <p className="text-sm leading-7 text-[var(--foreground-soft)]">{student.biggestRisk}</p>
                <div className="grid gap-3 md:grid-cols-2">
                  ...
                  <div className="ui-subtle-card px-4 py-3">
                    <p className="text-xs uppercase tracking-[0.18em] text-[var(--brand-blue)]">School mix</p>
                    <p className="mt-2 font-semibold">
              ...
              <div className="ui-ink-card flex h-full flex-col px-4 py-4 text-sm text-white">
                <div className="flex items-center gap-2 text-xs font-semibold uppercase tracking-[0.18em] text-white/72">
            ...
            >
              {workCards.map((task) => (
                <div
                  ...
                  className="flex h-full flex-col justify-between rounded-[1.5rem] border border-[var(--border)] bg-white/88 px-4 py-4"
                >
                  <div className="space-y-2">
                    ...
                    <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs font-semibold uppercase tracking-[0.18em] text-[var(--foreground-soft)]">
                      <span>{formatDisplayDate(task.dueDate)}</span>
                      <span>{task.category}</span>
                      <span>{task.parentVisible ? "Parent visible" : "Internal only"}</span>
                    </div>
                  </div>
                  <p className="mt-4 text-sm text-[var(--foreground-soft)]">{task.owner}</p>
                </div>
              ))}
            </div>
          </div>
